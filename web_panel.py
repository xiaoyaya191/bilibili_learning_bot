...
/* ── CHART GRID ── */
.chart-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px}
.chart-card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--r);padding:18px;transition:all .2s}
.chart-card:hover{border-color:var(--accent);box-shadow:0 0 20px rgba(91,141,239,.08)}
.chart-card h4{font-size:13px;margin:0 0 12px;color:var(--text);font-weight:500}
.chart-card canvas{max-height:220px;width:100%}

/* ── DISCLAIMER BAR ── */
.disclaimer{background:rgba(248,81,73,.08);border:1px solid rgba(248,81,73,.25);border-radius:var(--r);padding:10px 16px;margin-top:20px;font-size:11px;color:var(--red);text-align:center;line-height:1.6}
.disclaimer+.disclaimer,.page>.disclaimer{margin-top:16px}

/* ── FILE GRID ── */
.file-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:8px;font-size:12px}

/* ── FORM HINTS ── */
.form-hint{font-size:11px;color:var(--text2);margin-bottom:10px;line-height:1.6}

/* ── INLINE MSG ── */
.msg-inline{font-size:11px;margin-left:8px;transition:color .2s}

/* ── ENERGY GRID ── */
.energy-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}

/* ── KEYWORD TEXTAREA ── */
.kw-textarea{width:100%;height:120px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--rs);color:var(--text);font-size:12px;padding:8px;font-family:monospace;resize:vertical;transition:border-color .2s}
.kw-textarea:focus{border-color:var(--accent);box-shadow:0 0 8px rgba(91,141,239,.15);outline:none}

/* ── RADIO GROUP ── */
.radio-group{display:flex;align-items:center;gap:16px;flex-wrap:wrap}
.radio-group label{cursor:pointer;display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border-radius:var(--rs);border:1px solid var(--border);background:var(--bg2);font-size:13px;transition:all .2s}
.radio-group label:has(input:checked){border-color:var(--accent);background:rgba(91,141,239,.08);color:var(--accent)}
.radio-group input[type="radio"]{accent-color:var(--accent)}

/* ── CHAT LOG (Tutor) ── */
.chat-log{background:var(--bg3);border:1px solid var(--border);border-radius:var(--rs);padding:12px;max-height:350px;overflow-y:auto;font-size:12px;margin-bottom:10px;min-height:100px;scroll-behavior:smooth}
.chat-log::-webkit-scrollbar{width:4px}
.chat-log::-webkit-scrollbar-thumb{background:var(--border);border-radius:2px}
.chat-bubble{margin-bottom:8px;padding:8px 12px;border-radius:var(--rs);font-size:12px;line-height:1.6}
.chat-bubble.user{background:rgba(91,141,239,.1);border-left:3px solid var(--accent)}
.chat-bubble.ai{background:var(--bg2);border-left:3px solid var(--green)}

/* ── TUTOR INPUT AREA ── */
.tutor-input-wrap{display:flex;gap:6px;align-items:flex-end;flex-wrap:wrap}
.tutor-textarea{flex:1;min-width:180px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--rs);color:var(--text);font-size:12px;padding:8px;resize:none;height:50px;font-family:inherit;transition:border-color .2s}
.tutor-textarea:focus{border-color:var(--accent);box-shadow:0 0 8px rgba(91,141,239,.15);outline:none}
.tutor-btn-col{display:flex;flex-direction:column;gap:4px}

/* ── TUTOR SELECT ── */
.tutor-select{flex:1;min-width:250px;max-width:550px;padding:6px 8px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--rs);color:var(--text);font-size:12px}
.tutor-select:focus{border-color:var(--accent);outline:none}

/* ── PREVIEW / RESULT ── */
.preview-pre{background:var(--bg3);border:1px solid var(--border);border-radius:var(--rs);padding:12px;max-height:250px;overflow-y:auto;font-size:11px;color:var(--text2);white-space:pre-wrap;word-break:break-all;transition:all .3s}
.preview-pre::-webkit-scrollbar{width:4px}
.preview-pre::-webkit-scrollbar-thumb{background:var(--border);border-radius:2px}

/* ── DANGER ZONE ── */
.danger-zone{border-color:rgba(224,85,96,.3) !important}
.danger-zone h3{color:var(--red)}

/* ── ABOUT INFO GRID ── */
.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.info-card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--rs);padding:12px 16px;display:flex;flex-direction:column;gap:4px}
.info-card .il{font-size:10px;color:var(--text2);text-transform:uppercase;letter-spacing:.5px;font-weight:500}
.info-card .iv{font-size:15px;font-weight:600;color:var(--accent)}

/* ── QUICK FILTER ── */
.qfilter{display:flex;align-items:center;gap:8px;margin-bottom:12px;flex-wrap:wrap}
.qfilter input{flex:1;min-width:160px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--rs);color:var(--text);font-size:12px;padding:7px 10px;transition:border-color .2s}
.qfilter input:focus{border-color:var(--accent);outline:none}

/* ── EXPAND TOGGLE ── */
.expand-toggle{font-size:10px;color:var(--text2);cursor:pointer;user-select:none;transition:color .2s}
.expand-toggle:hover{color:var(--accent)}

/* ── SECTION DIVIDER ── */
.sec-divider{height:1px;background:var(--border);margin:16px 0;opacity:.5}

/* ── PAGE ANIMATIONS (continued) ── */
...
.tutor-input-wrap{flex-direction:column}
.tutor-btn-col{flex-direction:row}
.file-grid{grid-template-columns:repeat(auto-fill,minmax(110px,1fr))}
.qfilter{flex-direction:column}
.qfilter input{width:100%}
}
@media(max-width:400px){
.chart-card{padding:12px}
.chart-card canvas{max-height:180px}
.info-card{padding:10px 12px}
.info-card .iv{font-size:13px}
}
...
/* Card hover glow */
.sc:hover{transform:translateY(-3px);box-shadow:var(--shadow-lg),0 0 20px var(--glow);border-color:rgba(110,231,183,.15)}
.sc:hover .si{filter:drop-shadow(0 0 8px var(--glow))}
.pc:hover{border-color:rgba(110,231,183,.15);box-shadow:0 0 24px rgba(110,231,183,.06)}

/* Text readability over glass */
.ph h1{text-shadow:0 2px 8px rgba(0,0,0,.4)}
.ph p{text-shadow:0 1px 4px rgba(0,0,0,.3)}
.pc h3{text-shadow:0 1px 3px rgba(0,0,0,.3)}
.sv{text-shadow:0 2px 6px rgba(0,0,0,.3)}
.name,.sb-tt{text-shadow:0 1px 4px rgba(0,0,0,.3)}

/* Log box — deeper glass */
.log-box{background:rgba(5,8,15,.7);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.05)}

/* Form inputs — glass style */
.fg input,.fg textarea,.fg select{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);transition:all .3s}
.fg input:focus,.fg textarea:focus,.fg select:focus{background:rgba(255,255,255,.1);border-color:var(--accent);box-shadow:0 0 0 3px var(--glow)}

/* Buttons — glass upgrade */
.btn-out{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:var(--text2)}
.btn-out:hover{background:rgba(255,255,255,.12);border-color:var(--accent);color:var(--accent);box-shadow:0 0 12px var(--glow)}
.btn-grp .btn{backdrop-filter:blur(8px)}

/* Toggle switch — glow on active */
.toggle-sw input:checked+.toggle-track{box-shadow:0 0 16px var(--glow)}

/* Table — glass header */
.tb th{background:rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.08)}
.tb tr:hover td{background:rgba(255,255,255,.04)}

/* Chat log — glass */
.chat-log{background:rgba(5,8,15,.6);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.06)}

/* Toast — glass */
.toast{background:var(--glass);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--glass-border)}

/* QR area — glass */
.qr-wrap{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:var(--r);backdrop-filter:blur(8px)}

/* Sidebar hover — glow line */
.ni.ac{box-shadow:inset 3px 0 0 var(--accent),0 0 12px var(--glow)}

/* Keyword textarea — glass */
.kw-textarea{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1)}
.kw-textarea:focus{background:rgba(255,255,255,.1);border-color:var(--accent);box-shadow:0 0 12px var(--glow)}

/* Tutor textarea — glass */
.tutor-textarea{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1)}
.tutor-textarea:focus{background:rgba(255,255,255,.1);border-color:var(--accent);box-shadow:0 0 12px var(--glow)}

/* Select — glass */
.tutor-select{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1)}

/* Preview pre — glass */
.preview-pre{background:rgba(5,8,15,.6);border:1px solid rgba(255,255,255,.06);backdrop-filter:blur(8px)}

/* Info cards — glass */
.info-card{background:var(--glass);border:1px solid var(--glass-border);backdrop-filter:blur(12px)}

/* Radio group — glass */
.radio-group label{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1)}
.radio-group label:has(input:checked){background:var(--glow);border-color:var(--accent)}

/* Expand toggle */
.expand-toggle{transition:color .2s}
.expand-toggle:hover{color:var(--accent)}

/* Disclaimer */
.disclaimer{background:rgba(248,81,73,.06);border:1px solid rgba(248,81,73,.2);backdrop-filter:blur(8px)}

/* Danger zone */
.danger-zone{border-color:rgba(224,85,96,.25)!important}
.danger-zone h3{color:var(--red)}

/* Mon-grid — glass */
.mon-grid .pc{min-height:auto}

/* Badge */
.ni .bd{animation:badgePop .3s}

/* === THEME TRANSITION === */
body.theme-transition *{transition:all .5s cubic-bezier(.4,0,.2,1)!important}


/* ── TABLE SCROLL (mobile) ── */
@media(max-width:768px){
.tb-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:0 -20px;padding:0 20px}
.tb{min-width:600px}
}
...
// ── DISCLAIMER ──
function disclaimerHTML(){return '<div class="disclaimer">⚠ 免责声明：本项目仅供学习参考，若因使用本项目产生的任何后果，本人一律概不负责。</div>';}

// ── THEME SYSTEM (xiongdaa-card inspired) ──
function switchTheme(t){
document.body.classList.add('theme-transition');
document.documentElement.setAttribute('data-theme',t);
localStorage.setItem('xiongda_theme',t);
...
// ── AMBIENT CANVAS (lightweight particles) ──
(function(){
...
return{x:Math.random()*w,y:Math.random()*h,vx:(Math.random()-.5)*.3,vy:(Math.random()-.5)*.3,r:Math.random()*2+.5,a:Math.random()*.3+.1};
}
for(var i=0;i<40;i++)particles.push(createParticle());
function draw(){
if(document.hidden){requestAnimationFrame(draw);return}
ctx.clearRect(0,0,w,h);
var accent=getComputedStyle(document.documentElement).getPropertyValue('--accent').trim()||'#6ee7b7';
...
// draw connections
ctx.globalAlpha=.04;ctx.strokeStyle=accent;ctx.lineWidth=.5;
for(var i=0;i<particles.length;i++){
for(var j=i+1;j<particles.length;j++){
var dx=particles[i].x-particles[j].x;
var dy=particles[i].y-particles[j].y;
var dist=Math.sqrt(dx*dx+dy*dy);
if(dist<150){ctx.beginPath();ctx.moveTo(particles[i].x,particles[i].y);ctx.lineTo(particles[j].x,particles[j].y);ctx.stroke();}
}
}
ctx.globalAlpha=1;
